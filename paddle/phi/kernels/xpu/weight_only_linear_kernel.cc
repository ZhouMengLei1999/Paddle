  ...
  switch (x.dtype()) {
    case phi::DataType::FLOAT16: {
      ...
      if (weight_dtype == "int8") {
        ...
        tmp_tensor.Resize(weight.dims());
        dev_ctx.template Alloc<int32_t>(&tmp_tensor);
        r = baidu::xpu::api::cast_v2<int8_t, int32_t>(
            xpu_ctx->x_context(),
            weight.data<int8_t>(),
            tmp_tensor.data<int32_t>(),
            weight.numel());
        // w = (quanted_w - 128) * scale
        DenseTensor tmp_sub_tensor;
        tmp_sub_tensor.Resize({1});
        dev_ctx.template Alloc<int32_t>(&tmp_sub_tensor);
        r = baidu::xpu::api::constant(
            xpu_ctx->x_context(), tmp_sub_tensor.data<int32_t>(), 1, 128);
        r = baidu::xpu::api::broadcast_sub(xpu_ctx->x_context(),
                                           tmp_tensor.data<int32_t>(),
                                           tmp_sub_tensor.data<int32_t>(),
                                           tmp_tensor.data<int32_t>(),
                                           {n, k},
                                           {1});
        DenseTensor tmp_weight;
        tmp_weight.Resize(weight.dims());
        dev_ctx.template Alloc<int8_t>(&tmp_weight);
        r = baidu::xpu::api::cast_v2<int32_t, int8_t>(
            xpu_ctx->x_context(),
            tmp_tensor.data<int32_t>(),
            tmp_weight.data<int8_t>(),
            weight.numel());
        DenseTensor permuted_weight;
        permuted_weight.Resize(weight.dims());
        dev_ctx.template Alloc<int8_t>(&permuted_weight);
        r = baidu::xpu::api::permute_int8_weight_only(
            xpu_ctx->x_context(),
            tmp_weight.data<int8_t>(),
            permuted_weight.data<int8_t>(),
            n,
            k);
        PD_CHECK(r == 0, "baidu::xpu::api::permute_int8_weight_only failed.");
